...
bruit = np.random.normal(0,0.4,N)       # bruit gaussien
datay = f(datax) + bruit
...
W1  = np.random.rand(10,2)-0.5           # premiere couche de poids : 1 neurones d'entree + biais, 10 neurones caches
W2  = np.random.rand(11)-0.5            # deuxieme couche de poids : 10 neurones caches + biais, 1 sortie
...
# REGRESSION : la fonction y(x1)
entrees  = np.arange(-2,2,0.1)
sorties  = []

for xi in entrees:
        
    x = [1, xi]                  # entree avec biais 
    a = np.dot( W1, x)
    z = sigmoid(a)

    z = np.insert(z, 0, 1)       # neurones caches avec biais 
    y = np.dot(W2, z)
