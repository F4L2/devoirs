---
title: "QLife winter school - ENS 2019 : Network inference session"
author: 
- name: Vincent Cabeli, Hervé Isambert
  affiliation:  Institut Curie, UMR168, Isambert team
editor_options:
...
However when the tested interaction happens to fall in this class of models, the simpler methods will benefit from the smaller search space and will be able to pick up on subtler signals.
<!-- However, when we are right in our assumptions about the data, these restriced methods benefit from the smaller model space and are able to pick up on more subtle signal. -->
<!-- As seen from the other angle, universal measure such as mutual information have to consider that signal and noise may take any form.-->
...
<!-- For gene regulatory networks, we will see that more universal measures may be the better option.-->
...
discretizeMutual(X,Y,matrixU = matrix(Z, ncol=1), plot=F)$info
```
...
We will use the same dataset that was used in other practicals from the [Tabula Muris](https://tabula-muris.ds.czbiohub.org/) compendium, and different specialized librairies.
...
We performed the preliminary selection of T-cells among all tissues, and you are also given a list of informative transcription factors that are important for T-cell differentiation.

You can first load the data and make sure it has no obvious confonding biases by performing dimensionality reduction and checking if there are no clustering based on cell properties.
```{r tb_TSNE}
load("./data/single_cell_data.Rdata")
#load("./data/single_cell_data_small.Rdata")
...
```{r}
dropout_percent = apply(expr_df, 2, function(x){length(which(x==0)) / length(x)})
good_TFs = which(dropout_percent < 0.5)
...
```{r}
num_cores = 7
res.miic = miic(expr_df, nThreads = num_cores, confidenceShuffle = 100, confidenceThreshold = 0.01)
miic.plot(res.miic, igraphLayout = layout_nicely)
```

```{r tb_scale1}
res.miic = miic(scale_expr_df, nThreads = num_cores, confidenceShuffle = 100, confidenceThreshold = 0.01)
miic.plot(res.miic, igraphLayout = layout_nicely)
```
...
naive_res = data.frame(gene1=character(), gene2=character(), stringsAsFactors = F)
...
This method is left as an exercise for you to try to infer a network on the hematopoiesis dataset.
<!--
```{r inverse_cov, eval=FALSE}
covMat = cov(apply(scale_expr_df, 2, as.logical))
det(covMat)
...
inverse_covMat[inverse_covMat>-0.5] = 0

inverse_graph = graph_from_adjacency_matrix(abs(inverse_covMat))
plot(inverse_graph)
```
-->
...
```{r aracne}
mi_matrix = minet::build.mim(scale_expr_df)
...
```{r miic}
miic_res = miic(scale_expr_df, nThreads=num_cores)
#miic_graph = graph_from_edgelist(as.matrix(miic_res$retained.edges.summary[,1:2]))
#plot(miic_graph, layout=layout_nicely)
...
As an exercise, we will ask you to produce networks with HC, PC, miic, MMHC and GENIE3 so that the hematopoiesis gene networks have the same number of edges.
Call the help for each function with `?pc` to understand what are the tunable parameters.

```{r comparison, eval=FALSE}
# Export
write.table(hc_res$arcs, "./graphs/hc.csv")
write.table(pc_res$arcs, "./graphs/pc_5e-2.csv")
write.table(mmhc_res$arcs, "./graphs/mmhc_5e-1.csv")
```
